...
ROOT = HERE.parent                      # data-collection 폴더
...
def main():
    ...
    if not any(path for _, path in inputs):
        inputs = [('kstartup', ROOT / 'data/notices.json')]
    sources, metadata = [], []
    ...
    output = args.output or ROOT / 'data/normalized' / ('notices_' + now.strftime('%Y%m%dT%H%M%S%fZ') + '.json')
