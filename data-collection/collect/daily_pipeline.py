...
def _run(dry_run, skip_store, force, say,
    ...
    if stored and not skip_attach:
        say('첨부 수집')
        try:
            from collect import attachment_pipeline
    ...
    if stored and not skip_upload:
        say('벡터 업로드')
        try:
            from collect import upload_vectors
    ...
    if stored and not skip_files:
        say('첨부 파일 업로드')
        try:
            from collect import upload_attachments
    ...
    if stored and not skip_conditions:
        say('자격요건 추출')
        try:
            from collect import extract_conditions
