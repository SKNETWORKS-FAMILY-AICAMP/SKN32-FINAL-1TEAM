...
작업 PC  매일 09:00 · 작업 스케줄러 S-Brain-DailyCollection · 약 5분
   1  fetch.py               K-Startup 오픈API
   2  fetch_bizinfo.py       기업마당 오픈API
   3  normalize.py           두 출처를 공통 스키마로
   4  store_mysql.py         MySQL 저장 (notices·notice_attachments·import_runs)
   5  attachment_pipeline    첨부 다운로드 → 본문 추출 → attachment_texts
   6  embed.py               BGE-M3 벡터 → data/embeddings_v1.npz
   7  vecstore.py            로컬 Chroma 색인
   8  upload_vectors.py      벡터 → EC2 MySQL notices.embedding            ← 신규
   9  upload_attachments.py  첨부 원본 → EC2 MySQL attachment_files         ← 신규
  10  extract_conditions.py  공고문에서 자격요건 추출(LLM) → notice_conditions ← 신규
...
정형으로 주는 `notices.age_condition_raw` 를 쓴다 — `gate.py` 가 그렇게 한다.
...
목업의 `적합도 92/81/68` 은 실제 분포와 맞지 않는다. `app.py` 는 원값 + 3단 라벨
...
조정하려면 `extract_conditions.py` 의 상수를 고치고 `--redo` 로 재추출한다.
...
```
.\.venv\Scripts\python.exe -X utf8 app.py   →  http://127.0.0.1:8000
```
...
입력(신청자 유형·대표자명·설립일자·아이디어·팀원·수익모델)
  ...
  → 카드 클릭
  → POST /api/eligibility  gate.py 판정 4항목
```
...
| `ec2_vecstore.py` 접속 실패 | `.env` 를 읽는 코드가 없었음(환경변수만 봄) | `.env` 파서 + 빈 값 검사 |
...
행수가 DB 와 일치하는 것을 검산했다. `mysqldump` 가 없어 `backup_db.py` 를 만들었다.
...
`upload_attachments.py` 가 재생성한다. 포함하려면 `--include-files`.

**이 백업은 자격요건 추출 전 시점이다.** `notice_conditions` 가 없다. 새로 뜨려면
`python backup_db.py` 를 다시 돌린다(테이블 목록에 `notice_conditions` 를 추가해야 한다
— `CORE_TABLES` 상수).
...
| 파일 | 하는 일 |
|---|---|
| `upload_vectors.py` | 벡터 → EC2 MySQL. 증분. `--plan` `--limit` |
| `upload_attachments.py` | 첨부 파일 → EC2 MySQL. 증분. `--plan` `--limit` `--verify` |
| `extract_conditions.py` | LLM 자격요건 추출 + 검산. `--sample` `--save-db` `--redo` · 배치는 `run_batch()` |
| `gate.py` `test_gate.py` | 자격 판정. `finak_mok/poc` 에서 가져옴. 테스트 18개 |
| `app.py` `app.html` `review.html` | 시험용 화면 + 검토 화면 |
| `backup_db.py` | DB 백업. `--include-files` |
| `search_local.py` | PC 검색 시험. ⚠ 기본 엔진이 `mysql` — 2절 제약과 어긋나므로 `chroma` 로 바꿔야 함 |
| `ec2_vecstore.py` | **EC2 실행.** MySQL 벡터 → Chroma. `--plan` `--rebuild` `--stat` |
| `ec2_search.py` | **EC2 실행.** 대화형 벡터 검색 |
| `ec2_check_model.py` | **EC2 실행.** BGE-M3 메모리·속도 측정 |
...
```
daily_pipeline.py    8·9·10단계 편입 · --skip-upload --skip-files --skip-conditions
...
| `gate.py` 의 `지원대상 유형` 판정 | ❌ 무조건 `확인 필요`. `business_type`·`pre_startup_allowed` 를 쓰면 판정 가능 (`app.py` 의 `eligibility()` 참조) |
| `attachment_files` 중복 | EC2 파일시스템에 612MB 가 이미 있었다(9/9부터). MySQL 631MB 는 중복. 지우면 DB 672 → 41MB. 9단계를 rsync 로 바꾸는 선택지 |
| `search_local.py` 기본 엔진 | `mysql` → `chroma` 로 바꿔야 함 |
...
| 모집 상태 행 | `gate.py` 가 검사한다. 화면 요구는 3항목이라 뺄지 결정 |
| `app.py` EC2 배포 | 지금은 PC 전용. 포트 8000 개방 + systemd 필요 |
| Chroma 서버 모드 | 팀원이 벡터 DB 에 직접 붙으려면 `chroma run` + 포트 + 인증 필요 |
| `backup_db.py` 대상 | `notice_conditions` 가 `CORE_TABLES` 에 없다 |
...
.\.venv\Scripts\python.exe -X utf8 -m unittest test_normalize test_store_mysql test_pipeline test_doctext test_attachment_store test_gate
.\.venv\Scripts\python.exe -X utf8 daily_pipeline.py --dry-run          # 수집만. DB 안 건드림
.\.venv\Scripts\python.exe -X utf8 daily_pipeline.py --skip-conditions  # LLM 없이 전체
.\.venv\Scripts\python.exe -X utf8 app.py                               # 화면 + /review
.\.venv\Scripts\python.exe -X utf8 extract_conditions.py --sample 30    # 표본만. DB 안 씀
.\.venv\Scripts\python.exe -X utf8 backup_db.py
```

```bash
# EC2
ssh -i "C:/Users/playdata2/.ssh/skn32-1team.pem" ubuntu@43.201.90.238
cd ~/s-brain && .venv/bin/python ec2_vecstore.py --stat    # 색인 상태
cd ~/s-brain && .venv/bin/python ec2_search.py             # 대화형 검색
