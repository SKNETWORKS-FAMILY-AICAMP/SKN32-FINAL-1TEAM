...
code("# -*- coding: utf-8 -*-\n",
     ...
     "\n",
     "# 이 노트북을 어디서 띄우든(ml/ 에서든 data-collection/ 에서든 명령줄에서든)\n",
     "# 작업 폴더를 data-collection 으로 맞춘다. .env 의 인증서 경로가 상대경로라\n",
     "# 여기가 어긋나면 DB 접속부터 실패한다.\n",
     "ROOT = os.getcwd()\n",
...
code("import subprocess\n",
     "\n",
     "# ⚠ 색인 확인·생성은 반드시 **별도 프로세스**에서 한다.\n",
     "# 이 커널에서 색인을 열어둔 채로 다시 만들면, 옛 색인을 가리키던 손잡이가 남아\n",
     "# 나중에 검색할 때 \"그런 색인 없다\" 오류가 난다.\n",
     "store = os.path.join(ROOT, 'data', 'vecstore', 'chroma')\n",
     "\n",
     "def index_count():\n",
     "    snippet = ('import chromadb, sys; '\n",
     "               \"print(chromadb.PersistentClient(path=sys.argv[1])\"\n",
     "               \".get_collection('notices_v1').count())\")\n",
     "    r = subprocess.run([sys.executable, '-c', snippet, store],\n",
     ...
     "count = index_count()\n",
     "if count < 100:\n",
     "    print('색인이 없거나 비어 있다(%d건). EC2 DB 의 벡터로 새로 만든다...' % count)\n",
     "    r = subprocess.run([sys.executable, '-X', 'utf8', 'ec2_vecstore.py', '--rebuild'],\n",
     "                       capture_output=True, text=True, cwd=ROOT)\n",
     "    print(r.stdout.strip() or r.stderr.strip()[-500:])\n",
     "    count = index_count()\n",
     "\n",
     "print('벡터 색인 %d건 준비됨' % count)\n",
     "assert count >= 100, '색인 생성에 실패했다. 터미널에서 python ec2_vecstore.py --rebuild 를 직접 돌려본다.'"),
