...
code("results = []\n",
     ...
     "    print('%-26s P@3(2) %.3f · 판정칸만 %s · nDCG@3 %.3f · 미판정 %.3f'\n",
     "          % (label, s['p3_rec'],\n",
...
code("print('%-26s %-9s %-15s %-9s %-9s %-9s'\n",
     "      % ('시스템', 'P@3(2)', '95% 구간', '판정칸만', 'nDCG@3', '미판정@3'))\n",
     "print('-' * 86)\n",
     "for s in results:\n",
     "    ci = s['p3_rec_ci95']\n",
     "    judged = s['p3_rec_judged']\n",
     "    print('%-26s %-9.3f %-15s %-9s %-9.3f %-9.3f'\n",
     "          % (s['system'], s['p3_rec'],\n",
     ...
     "\n",
     "print()\n",
     "print('미판정@3 가 0.10 을 넘으면 그 줄의 P@3(2) 는 믿지 않는다 (evaluate.py 원칙).')\n",
     "print('판정칸만 = 미판정을 빼고 판정된 칸에서만 본 「딱 맞음」 비율.')"),
