...
code("class RerankSystems(ev.Systems):\n",
     ...
     "        self.elapsed = []\n",
     "\n",
     "    def notice_texts(self, ids):\n",
     "        missing = [i for i in ids if i not in self._text_cache]\n",
     "        if missing:\n",
     "            for nid, n in rc.load_notices(missing).items():\n",
     "                self._text_cache[nid] = rc.notice_text(n)\n",
     "        return [self._text_cache.get(i, '') for i in ids]\n",
     "\n",
     ...
     "            return []\n",
     "        t0 = time.time()\n",
     "        scores = self.scorer.score(common.query_text(q), self.notice_texts(ids))\n",
...
code("results = []\n",
     ...
     "    s['_per'] = per\n",
     "    results.append(s)\n",
     "    print('%-26s P@3(2) %.3f · nDCG@3 %.3f · 미판정 %.3f'\n",
     "          % (label, s['p3_rec'], s['ndcg3'], s['unjudged3']))\n",
     "    return s\n",
     "\n",
     "plain = ev.Systems()\n",
...
code("t0 = time.time()\n",
     ...
     "\n",
     "base_sys = RerankSystems(base_scorer)\n",
...
code("tuned_scorer = rc.Scorer(adapter=rc.ADAPTER_DIR)\n",
     "tuned_sys = RerankSystems(tuned_scorer)\n",
...
code("print('%-26s %-9s %-16s %-9s %-9s' % ('시스템', 'P@3(2)', '95% 구간', 'nDCG@3', '미판정@3'))\n",
     "print('-' * 76)\n",
     "for s in results:\n",
     "    ci = s['p3_rec_ci95']\n",
     "    print('%-26s %-9.3f %-16s %-9.3f %-9.3f'\n",
     "          % (s['system'], s['p3_rec'],\n",
...
code("base = results[1]      # rrf 를 기준으로 삼는다\n",
     "print('기준 : %s' % base['system'])\n",
     "print()\n",
     "for s in results:\n",
     "    if s is base:\n",
     "        continue\n",
     "    d = ev.paired_diff([r['p3_rec'] for r in base['_per'] if r['kind'] == 'normal'],\n",
     "                       [r['p3_rec'] for r in s['_per'] if r['kind'] == 'normal'])\n",
     "    if not d:\n",
     "        continue\n",
     "    mid, lo, hi = d\n",
     "    verdict = '개선이라 볼 수 있다' if lo > 0 else ('나빠졌다' if hi < 0 else '차이를 단정할 수 없다')\n",
