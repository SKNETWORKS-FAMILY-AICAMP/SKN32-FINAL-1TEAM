...
from collect.normalize import normalize_notice, normalize_sources, rows_from, text
...
class NormalizeTests(unittest.TestCase):
    ...
    def test_cli_refuses_existing_output(self):
        with tempfile.TemporaryDirectory() as folder:
            ...
            # 패키지가 된 뒤로는 모듈로 실행한다 (python -m collect.normalize)
            root = Path(__file__).resolve().parent.parent
            command = [sys.executable, '-X', 'utf8', '-m', 'collect.normalize',
                       '--kstartup', str(source), '--output', str(output)]
            run = lambda: subprocess.run(command, capture_output=True, cwd=str(root))
