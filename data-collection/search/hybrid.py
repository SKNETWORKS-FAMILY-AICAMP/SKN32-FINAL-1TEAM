...
def rrf(*ranked_lists, k=RRF_K, weights=None):
    ...
    weights 를 주면 심사위원마다 표의 무게가 달라진다(예: (1.5, 1.0) 이면 의미 검색을
    1.5배로 본다). **기본값 None 은 전부 1.0 이라 예전과 완전히 같다.**
    """
    if weights is None:
        weights = [1.0] * len(ranked_lists)
    if len(weights) != len(ranked_lists):
        raise ValueError('weights 개수가 결과 목록 수와 다르다')
    fused = {}
    for hits, weight in zip(ranked_lists, weights):
        for rank, (nid, _) in enumerate(hits, 1):
            fused[nid] = fused.get(nid, 0) + weight / (k + rank)
