...
def business_age_months(founded, today=None):
    """설립일 → 업력(개월). 미설립이거나 날짜로 읽을 수 없으면 None.

    형식이 이상한 값에 예외를 던지지 않는다. API 로 들어온 문자열이 그대로 여기까지
    오기 때문에, 던지면 요청 전체가 500 으로 끝난다(2026-09-18 회귀 테스트에서 발견).
    "모른다" 는 뜻은 호출하는 쪽에서 gate.UNKNOWN_AGE 로 구분한다.
    """
    if not founded:
        return None
    today = today or date.today()
    if isinstance(founded, str):
        founded = parse_ymd(founded)
        if founded is None:
            return None
...
def parse_ymd(s):
    ...
    **형식이 맞아도 달력에 없는 날짜가 있다**(2026-02-30, 2026-13-01, 윤년 아닌 2025-02-29).
    숫자를 날짜로 만드는 단계에서 예외가 나므로 여기서 잡아 None 으로 돌려준다.
    사용자 입력이 그대로 여기까지 오기 때문에, 던지면 요청 전체가 500 으로 끝난다
    (2026-09-18 Codex 재검토에서 지적).
    """
    s = str(s or '').strip()
    try:
...
# 설립일을 받지 못한 사업자. **예비창업자(미설립)와 다르다.**
# 예비창업자는 "아직 설립하지 않았다" 는 사실이고, 이 값은 "설립일을 모른다" 는 뜻이다.
# 같은 None 으로 다루면 업력을 모르는 사업자를 '예비창업자 전용 공고 신청 불가' 로 잘못 판정한다
# (2026-09-18 Codex 검토 4번).
UNKNOWN_AGE = 'unknown'
...
def _check_age(age_raw, age_months):
    ...
    age_months  숫자        설립 후 개월 수
                None        예비창업자 (미설립)
                UNKNOWN_AGE 사업자인데 설립일을 모름 → 업력은 판단 불가
    """
    if age_months is UNKNOWN_AGE:
        return None, '설립일이 없어 업력을 확인할 수 없음', '설립일 미상'
...
def judge(notice, age_months, today=None):
    ...
    age_months = None 이면 예비창업자(미설립), UNKNOWN_AGE 면 설립일을 모르는 사업자다.
