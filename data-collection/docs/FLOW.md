...
이 문서는 기능 흐름을 설명합니다. 진행 상황은 [STATUS.md](STATUS.md),
변경 이유·검증 결과는 [WORKLOG.md](WORKLOG.md), AI 공통 규칙은 [AGENTS.md](../AGENTS.md)를 봅니다.
아래 기존 배치 설명의 건수·시간은 과거 기록이며 현재 실행 결과를 뜻하지 않습니다.
...
```
  ...
  notices.json  raw/bizinfo   normalized/notices   MySQL
        └──────────┴──────────→ ─────┘                │
                        ...
                        ─────────────────────────────────────
                             shared/embed.py  ──→  search/vecstore.py
                                │                │
                                ▼                ▼
                       embeddings_v1.npz   data/vecstore/chroma/
                                │
                                ▼
                        collect/upload_vectors.py  ──→  MySQL notices.embedding

                     collect/upload_attachments.py ──→  MySQL attachment_files
```

`collect/daily_pipeline.py` 는 **직접 일하지 않습니다.** 순서를 정하고 결과를 받아 기록만 합니다.
...
daily_pipeline
   ...
   ▼
collect/daily_job.py
   │  fetch.fetch_all(키) 를 부른다
   ▼
collect/fetch.py           오픈API 를 100건씩 세 번 호출
   │
...
daily_pipeline
   ...
   ▼
collect/fetch_bizinfo.py    오픈API 를 한 번 호출 → 1,553건이 통째로 온다
   │
...
daily_pipeline
   ...
   ▼
shared/store_mysql.py
   │  shared/config.py 에서 접속 정보를 읽어 EC2 에 붙는다 (TLS)
...
daily_pipeline
   ...
   ▼
collect/attachment_pipeline.py
   │
   ...
   │
   ② 한 건씩 내려받는다 (1초 간격)
   │     collect/doctext.py  →  collect/hwp5.py       형식 판별 · 본문 추출
   │
...
전에는 `notices` 에 임베딩 컬럼이 없어 벡터가 배치 PC 에만 있었습니다.
`db/mysql_migration_002_embedding.sql` 로 컬럼 5개를 추가하고, 8단계에서 올립니다.
...
```
daily_pipeline
   │  upload_vectors.run() 을 부른다
   ▼
collect/upload_vectors.py
   │
   ① DB 에서 이미 올라간 것을 읽는다
   │     notice_id · embedding_input_sha256 · embedding_fingerprint
   │
   ② npz 와 대조해 올릴 것을 고른다
   │     DB 에 없다      → 아직 안 올라갔다.      올린다
   │     입력 해시 다름  → 벡터를 다시 만들었다.  올린다
   │     지문 다름       → 설정이 바뀌었다.       올린다
   │     셋 다 같다      → 이미 올라갔다.         건너뛴다
   │
   ③ 100건씩 UPDATE. 한 묶음마다 커밋한다
   │
   └─→  MySQL  notices.embedding  (float32 리틀엔디안 4,096 바이트)
```

**6단계와 판정 기준이 같습니다.** 입력 해시와 설정 지문 두 개뿐입니다.
그래서 매일 드는 비용은 그날 새로 만들어진 수십 건입니다. 실측으로
2,050건 전량 업로드가 8MB, 신규 100건이면 400KB 입니다.

돌려받는 것:

```python
{'local': 2050, 'remote': 2050, 'todo': 0, 'uploaded': 0,
 'skipped_no_notice': 0, 'dim': 1024}
```

### 6단계가 실패한 날에도 따라잡습니다

`changed_ids` 를 받지 않고 **DB 와 npz 를 직접 대조**합니다. 그래서 임베딩이
실패하거나 `--skip-embed` 로 건너뛴 날에도, 지난번에 못 올린 것이 있으면
여기서 올라갑니다. 7단계가 색인의 빠진 것을 찾아 넣는 것과 같은 방식입니다.

### 공고가 DB 에 없는 벡터는 건너뜁니다

`UPDATE ... WHERE notice_id = %s` 이므로 공고가 없으면 0행이 갱신됩니다.
**벡터가 공고를 새로 만들지는 않습니다.** 그 건수는 `skipped_no_notice` 로 보고합니다.

### 색인은 올리지 않습니다

```
notices.embedding     벡터 8MB      → 올린다. 이것만 있으면 된다
data/vecstore/chroma  색인 39.5MB   → 올리지 않는다
```

색인은 벡터에서 언제든 다시 만들 수 있는 파생물입니다. 그리고 2,050건 ×
1024차원이면 `M @ q` 한 번이 10ms 라서, 읽는 쪽은 색인 없이 numpy 만으로
검색할 수 있습니다. 실측으로 **DB 벡터만 쓴 검색이 Chroma 와 상위 5건·점수까지
똑같았습니다.**

```python
M = np.vstack([np.frombuffer(blob, dtype='<f4') for blob in blobs])
scores = M @ query_vector          # 정규화돼 있어 내적이 곧 코사인
```

---

## 9단계 — 첨부 원본 파일을 공용 DB 로

```
daily_pipeline
   │  upload_attachments.run() 을 부른다
   ▼
collect/upload_attachments.py
   │
   ① data/attachments/ 를 훑는다. 파일명이 곧 content_sha256 이다
   │
   ② DB 의 attachment_files 해시 목록과 뺄셈한다
   │     해시가 없다  → 올린다
   │     해시가 있다  → 같은 파일이다. 건너뛴다
   │
   ③ 읽은 바이트를 다시 해시해 파일명과 맞는지 본다
   │     다르면 올리지 않는다. 상한 파일을 공용 DB 에 넣지 않는다
   │
   ④ 8MB 쌓일 때마다 커밋한다
   │
   └─→  MySQL  attachment_files  (content_sha256 · ext · bytes)
```

**해시가 곧 내용입니다.** 그래서 같은 파일을 여러 공고가 첨부해도 한 번만
저장되고, 중간에 끊겨도 다시 실행하면 이어서 올립니다. 620MB 를 한 번에
보내다 끊기는 상황을 전제로 만들었습니다.

커밋을 8MB 단위로 하는 이유는 **끊겼을 때 잃는 양의 상한**을 정하려는 것입니다.
전부 한 트랜잭션에 담으면 620MB 를 보내다 마지막에 끊기면 전부 날아갑니다.

돌려받는 것:

```python
{'local': 1646, 'remote': 1646, 'todo': 0,
 'uploaded': 0, 'sent_mb': 0.0, 'hash_mismatch': 0}
```

### 왜 파일시스템이 아니라 DB 인가

팀원 전원이 이미 이 DB 접속 정보를 가지고 있습니다. 서버에 파일 서버를 따로
올리고 계정을 나눠주는 것보다 지금 당장 쓸 수 있습니다. `attachment_texts` 와
같은 `content_sha256` 으로 조인되니 읽는 쪽 코드도 단순합니다.

**파일이 GB 단위로 늘어나면 S3 같은 객체 저장소로 옮기는 것이 맞습니다.**
지금은 620MB 이고 하루 증가분이 십여 MB 라 DB 로 충분합니다.

### 실패한 첨부의 파일은 조인으로 못 찾습니다

`attachment_texts.content_sha256` 은 **성공했을 때만** 채워집니다. 그래서
...
8단계 실패 → 벡터는 로컬에 있다. 다음 실행이 DB 와 대조해 못 올린 것을 올린다
...
```powershell
# 저장 없이 1~3단계만
.\.venv\Scripts\python.exe -X utf8 -m collect.daily_pipeline --dry-run

# 첨부만 따로, 대상만 세어 보기
.\.venv\Scripts\python.exe -X utf8 -m collect.attachment_pipeline --plan

# 임베딩을 만들 것이 몇 건인지
.\.venv\Scripts\python.exe -X utf8 -m shared.embed --plan

# 색인 상태
.\.venv\Scripts\python.exe -X utf8 -m search.vecstore --stat
```
