...
// 받아온다. 파이프라인이 더미로라도 한 번에 끝까지 돌아가는 구조라(app/routers/projects.py
// generate_pipeline_result 주석 참고) 실제로 관측되는 진행률은 0(아직 공고 미선택) 아니면
// 100(stage==='done') 둘 중 하나뿐이다 — 중간 화면에서 이어하기는 지원하지 않는다.
export function Dashboard({onNewProject,onOpenProject,notifyEnabled,alerts,isNewUser,onToggleNewUser}){
 const [query,setQuery]=useState('');const [filter,setFilter]=useState('전체');const [guard,setGuard]=useState(false);
 const [realProjects,setRealProjects]=useState([]);const [loading,setLoading]=useState(true);const [loadError,setLoadError]=useState(false);
 ...
 const projects=isNewUser?[]:realProjects;
 ...
 const start=()=>{if(projects.some(p=>p.progress<100))setGuard(true);else onNewProject()};

 return <div className="dashboard">
  <div className="dashboard-heading"><div><p>내 프로젝트</p><h1>{isNewUser?'첫 아이디어를 들려주세요':'김창업 님, 이어서 준비해 볼까요?'}</h1></div><button className="btn" onClick={start}><Icon name="plus" size={19}/>새 프로젝트</button></div>

  {!isNewUser&&loading&&<p className="workspace-note">내 프로젝트를 불러오는 중이에요…</p>}
  {!isNewUser&&loadError&&<p className="workspace-note">프로젝트 목록을 불러오지 못했어요. 새로고침해 주세요.</p>}

  {!isNewUser&&!loading&&inProgress&&<button className="continue-card" onClick={()=>onOpenProject(inProgress)}><span className="continue-icon"><Icon name="file" size={32}/></span><div><p>이어서 준비하기</p><h2>{inProgress.name}</h2><span>공고 선택부터 이어서 진행해요</span></div><div className="continue-status"><span>매칭 대기 중</span><b>이어서 진행하기 <Icon name="chevron" size={19}/></b></div></button>}
  ...
  <div className="projects-heading"><h2>전체 프로젝트 <span>{projects.length}</span></h2><label className="demo-toggle"><input type="checkbox" checked={isNewUser} onChange={e=>onToggleNewUser(e.target.checked)}/>신규 사용자 보기</label></div>
  ...
  <div className="project-list">
   ...
   {isNewUser
    ...
    ? <div className="new-user"><span className="new-user-symbol"><Icon name="folder" size={45}/></span><h3>아직 프로젝트가 없어요</h3><p>아이템을 알려주시면 맞는 공고부터 찾아드릴게요.</p><button className="btn" onClick={onNewProject}>첫 프로젝트 만들기</button></div>
