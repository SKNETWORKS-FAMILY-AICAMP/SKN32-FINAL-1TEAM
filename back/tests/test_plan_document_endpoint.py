...
# ============================================================================
# GET /projects/{id}/attachment-guide.docx (SB-59, 2026-09-18)
# ============================================================================

def test_attachment_guide_early_general_returns_valid_docx(authed_client):
    r = authed_client.post('/projects', data={'payload': json.dumps(_payload(applicant_type='corp'))})
    project_id = r.json()['project_id']

    r = authed_client.get(f'/projects/{project_id}/attachment-guide.docx')
    assert r.status_code == 200, r.text
    assert r.headers['content-type'] == (
        'application/vnd.openxmlformats-officedocument.wordprocessingml.document'
    )
    ...
    doc = Document(io.BytesIO(r.content))
    table_text = '\n'.join(c.text for t in doc.tables for row in t.rows for c in row.cells)
    assert '신분증 사본' in table_text


def test_attachment_guide_preliminary_returns_404_until_file_added(authed_client):
    # 예비창업패키지용 원본 파일은 아직 없다 — app/routers/projects.py
    # _ATTACHMENT_GUIDE_FILES 주석 참고. 받으면 그 키만 추가하면 된다.
    r = authed_client.post('/projects', data={'payload': json.dumps(_payload(applicant_type='preliminary'))})
    project_id = r.json()['project_id']
...
def test_attachment_guide_requires_ownership(login_as):
    owner = login_as('guide-owner@example.com', '주인')
    r = owner.post('/projects', data={'payload': json.dumps(_payload())})
    project_id = r.json()['project_id']

    other = login_as('guide-other@example.com', '남')
    r = other.get(f'/projects/{project_id}/attachment-guide.docx')
    assert r.status_code == 404
